...
void AEnemy::DetectorOverlapEnd(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	APlayerCharacter* Player = Cast<APlayerCharacter>(OtherActor);
...
void AEnemy::TakeDamage(int DamageAmount, float StunDuration)
{
	...
	if (HitPoints <= 0)
	{
		...
		GetAnimInstance()->JumpToNode(FName("JumpDie"), FName("enemy1StateMachine"));
		GetAnimInstance()->JumpToNode(FName("JumpDie"), FName("enemy3StateMachine"));
	...
	else
	{
		...
		GetAnimInstance()->JumpToNode(FName("JumpTakeHit"), FName("enemy1StateMachine"));
		GetAnimInstance()->JumpToNode(FName("JumpTakeHit"), FName("enemy3StateMachine"));
	}
}
